...
void blob_nchw_112(const cv::Mat& bgr, float* dst) {
  ...
  for (int y = 0; y < h; ++y) {
    ...
    for (int x = 0; x < w; ++x) {
      const int i = y * w + x;
      dst[0 * hw + i] = (static_cast<float>(row[x][2]) - mean) / stdv;
      dst[1 * hw + i] = (static_cast<float>(row[x][1]) - mean) / stdv;
      dst[2 * hw + i] = (static_cast<float>(row[x][0]) - mean) / stdv;
    }
  }
}
...
std::vector<Embedding> ArcFace::embed_batch(const std::vector<cv::Mat>& aligned) {
  ...
  std::vector<float> blob(static_cast<size_t>(n * plane));
  for (int i = 0; i < n; ++i) {
